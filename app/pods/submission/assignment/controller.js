...
export default Ember.Controller.extend(ResponseErrorMixin, HtmlHelpers, {
  ...
  deadLineDateObserver: function() {
    var localStorage = this.get("storage"),
      ...
      assignmentKey = "a" + assignmentNumber;
    ...
    var dueDate = new Date(deadlines[assignmentKey].deadline);
    ...
    if (today > dueDate) {
      var graceDate = dueDate;
      graceDate.setMinutes(dueDate.getMinutes() + 5);
      if (graceDate > today) {
        this.set("pastDeadline", false);
        this.get('enableDisableFileUploader').call(this, true);
        this.get("pastDeadlineWithGrace").call(this);
        this.set("displayPastDeadlineMessage","");
      } else {
  ...
  hasPastDeadline: function() {
    this.set("pastDeadline", true);
  },
  pastDeadlineWithGrace: function() {
    this.set("displayCountDown", "Deadline passed. Wrap up.");
    var self = this;
    ...
    var showRemaining = function() {
      ...
      if (distance < 0) {
        clearInterval(self.get("countDownTimer"));
        clearInterval(timer);
        self.set("displayCountDown", "");
        self.set("pastDeadline", true);
        self.set("displayCountDown", new Ember.Handlebars.SafeString("<div style='color:red'><i class='material-icons left' style='padding-top:7px'>assignment_late</i> Due date has passed </div>"));
        self.get('enableDisableFileUploader').call(this, false);
        //self.get("hasPastDeadline").call(self);
        return;
      }
  ...
  startCountdown: function() {
    ...
    var showRemaining = function() {
      ...
      if (distance < 0) {
        clearInterval(self.get("countDownTimer"));
        self.get("pastDeadlineWithGrace").call(self);
        return;
      }
